...
A later repository audit identified additional historically named `_tw` paths that were not explicitly represented in this historical table. Their presence does not prove active TW-client support, but the active roadmap requires a refreshed runtime-reference inventory before this historical claim can be treated as current validation.
...
class ImageExtractor:
    def __init__(self, module, file):
        ...
        self.area = self.color = self.button = self.file = None
        self.load()

    def get_file(self, genre=''):
        for ext in ['.png', '.gif']:
            file = f'{self.name}.{genre}{ext}' if genre else f'{self.name}{ext}'
            path = os.path.join(AzurLaneConfig.ASSETS_FOLDER, SERVER, self.module, file).replace('\\', '/')
        ...
        if os.path.splitext(file)[1] == '.gif':
            ...
            for image in imageio.mimread(file):
                ...
                if mean is None:
                    mean = new_mean
            return bbox, mean
        return self._extract(load_image(file), file)
    ...
    def _extract(image, file):
        ...
        bbox = get_bbox(image)
        mean = tuple(np.rint(get_color(image=image, area=bbox)).astype(int))
        return bbox, mean

    def load(self):
        file = self.get_file()
        if not os.path.exists(file):
            raise FileNotFoundError(f'EN asset missing: {file}')
        ...
        self.area, self.color, self.button, self.file = area, color, button, file

    @property
    def expression(self):
        return f'{self.name} = Button(area={self.area!r}, color={self.color!r}, button={self.button!r}, file={self.file!r})'
...
class ModuleExtractor:
    def __init__(self, name):
        self.name = name
        self.folder = os.path.join(AzurLaneConfig.ASSETS_FOLDER, SERVER, name)
...
class AssetExtractor:
    """Generate EN-only ``module/**/assets.py`` files from ``assets/en``."""
    def __init__(self):
        logger.info('EN assets extract')
        root = os.path.join(AzurLaneConfig.ASSETS_FOLDER, SERVER)
        modules = [m for m in os.listdir(root) if os.path.isdir(os.path.join(root, m))]
